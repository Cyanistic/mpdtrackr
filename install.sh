...
install() {
	if [ "$(uname)" = "Darwin" ]; then
		...
		read "Unknown OS\nInstall Linux binary anyways? y/N: " input
		if [ "$input" = "y" ]; then
			wget 'https://github.com/Cyanistic/mpdtrackr/releases/latest/download/mpdtrackr-x86_64-unknown-linux-gnu' -O mpdtrackr
		else
			exit
	...
	if [ "$(uname)" = "Linux" ]; then
		read "Install and enable mpdtrackr systemd service?\n(This makes mpdtrackr automatically start on boot)? Y/n:" input
		if [ "$input" != "n" ]; then
...
uninstall() {
	...
	if [ "$(uname)" = "Linux" ]; then
		read "Delete and disable mpdtrackr systemd service? Y/n:" input
		if [ "$input" != "n" ]; then
			if [ "$(id -u)" -ne 0 ]; then
