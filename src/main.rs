...
async fn main() {
    let args = Args::parse();
    let config_file_dir_literal = dirs::config_dir().unwrap().to_str().unwrap().to_string() +"/mpdtrackr/config.json";
    ...
    let mongo_client_options = ClientOptions::parse(format!("{}{}", "mongodb://localhost:", config["mongo_port"])).await.unwrap();
    ...
    match args {
        Args{ import: Some(files), output: _, print: _ } => lib::import(mongo_client, files),
        Args{ import: _, output: Some(files), print: _ } => lib::output(mongo_client, files),
        Args{ import: _, output: _, print: true } => lib::print(mongo_client).await,
